...
type SysPermission struct {
	...
	Code     string   `json:"code" gorm:"column:code;type:varchar(128);not null;uniqueIndex:uk_permission_code;comment:权限唯一编码"`
	Name     string   `json:"name" gorm:"column:name;type:varchar(128);not null;comment:权限名称"`
	Type     string   `json:"type" gorm:"column:type;type:varchar(16);not null;comment:权限类型(menu/button/api/page)"`
	ParentId int      `json:"parentId" gorm:"column:parent_id;type:int;default:0;comment:父权限ID（权限树）"`
